...
### CUDA Configuration Requirements

This project is tested with the following CUDA software stack:

- NVIDIA Driver: recent driver version (recommended 550+)
- PyTorch: `2.3.0+cu121`
...
Use the provided setup script on Windows:
...
1. Create `.venv`
2. Install CUDA-enabled PyTorch and DGL
3. Install all required packages
4. Verify CUDA and package versions

### GPU Compatibility Note

Some very new GPUs may not be compatible with this pinned DGL/PyTorch CUDA stack and can raise:

`RuntimeError: CUDA error: no kernel image is available for execution on the device`

To avoid crashes, `main.py` supports automatic fallback to CPU.
...
Or force CUDA explicitly (fails fast if CUDA is unavailable/incompatible):
...
6. **device**, you can choose one of `auto`, `cuda` or `cpu`. Default is `auto`.
## Dataset Preparation
